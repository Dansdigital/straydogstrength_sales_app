...
import { MapboxGeocoder, MapboxHTMLEvent, Theme } from '@mapbox/search-js-web';
import { GeocodingOptions, SearchSession } from '@mapbox/search-js-core';

import mapboxgl from 'mapbox-gl';

import { Geocoder } from '../../src';
import { GeocoderRefType } from '../../src/components/Geocoder';

const OPTIONS: Partial<GeocodingOptions> = { language: 'de' };
  ...
  it('renders', () => {
    ...
    const { baseElement, rerender, unmount } = render(
      <Geocoder
        ...
        componentOptions={{ flipCoordinates: true }}
        marker={{ color: 'orange' }}
        mapboxgl={mapboxgl}
    ...
    const search = baseElement.querySelector<MapboxGeocoder>('mapbox-geocoder');
    ...
    expect(search.componentOptions).toEqual({
    ...
    search.dispatchEvent(new MapboxHTMLEvent('suggest', { features: [] }));
    ...
    search.dispatchEvent(new MapboxHTMLEvent('retrieve', { properties: {} }));
    ...
    const ref = React.createRef<GeocoderRefType>();
    rerender(
      <Geocoder
        ref={ref}
        accessToken="xyz"
    ...
    unmount();
    expect(oldRef.focus).toThrowError('Geocoder is not mounted');
    expect(oldRef.search).toThrowError('Geocoder is not mounted');
  });
});
