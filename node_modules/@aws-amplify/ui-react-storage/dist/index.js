...
const FileUploaderBase = React__namespace.forwardRef(function FileUploader({ acceptedFileTypes = [], accessLevel, autoUpload = true, bucket, components, defaultFiles, displayText: overrideDisplayText, isResumable = false, maxFileCount, maxFileSize, onFileRemove, onUploadError, onUploadStart, onUploadSuccess, path, processFile, showThumbnails = true, useAccelerateEndpoint, }, ref) {
    ...
    uiReactCore.useSetUserAgent({
        ...
        version: createStorageBrowser.VERSION,
    });
...
const StorageImage = ({ accessLevel, bucket, className, fallbackSrc, identityId, imgKey, path, loadingElement, onStorageGetError, onGetUrlError, validateObjectExistence = true, ...rest }) => {
    ...
    const { url, isLoading } = uiReactCore.useGetUrl(input);
    if (isLoading) {
        return !loadingElement ? null : React__namespace.createElement(React__namespace.Fragment, null, loadingElement);
    }
...
function StorageBrowser(props) {
    const { StorageBrowser: StorageBrowserComponent } = React__namespace["default"].useRef(createStorageBrowser.createStorageBrowser({ config: createStorageBrowser.createAmplifyAuthAdapter() })).current;
